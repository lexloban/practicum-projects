...
// +(A / B) - always okay (the resulting binary op has the highest grammatic precedence)
    ...
    public:
        ...
        virtual double Evaluate(std::function<const Cell*(Position)> get_cell) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<const Cell*(Position)> get_cell) const override {
                double l = lhs_.get()->Evaluate(get_cell);
                double r = rhs_.get()->Evaluate(get_cell);

                switch (type_) {
                    case Add:
                        if (std::isinf(l + r)) { throw FormulaError(FormulaError::Category::Div0); }
                        return l + r;
                    case Subtract:
                        if (std::isinf(l - r)) { throw FormulaError(FormulaError::Category::Div0); }
                        return l - r;
                    case Multiply:
                        ...
                        return l * r;
                    case Divide:
                        if (std::isinf(l / r) || r == 0.0) { throw FormulaError(FormulaError::Category::Div0); }
                        return l / r;
                }
                return 0;
            }
        ...
        public:
            ...
            double Evaluate(std::function<const Cell*(Position)> get_cell) const override {
                switch (type_) {
                    case UnaryPlus: return operand_->Evaluate(get_cell);
                    case UnaryMinus: return -operand_->Evaluate(get_cell);
                }
                return 0;
            }
        ...
        public:
            ...
            double Evaluate(std::function<const Cell*(Position)> get_cell) const override {
                const auto& cell = get_cell({cell_->row, cell_->col});
...
double FormulaAST::Execute(const SheetInterface& sheet) const {
    auto lamda = [&](Position pos) {
        return dynamic_cast<const Cell*>(sheet.GetCell(pos));
    };

    std::function<const Cell*(Position)> func = lamda;
