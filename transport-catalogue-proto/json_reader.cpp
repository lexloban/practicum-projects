...
SerializationSettings JsonReader::GetSerializationSettings() {
    auto& map = json_doc_.GetRoot().AsDict().at("serialization_settings").AsDict();
    return {map.at("file").AsString()};
}

void JsonReader::OutputStat(transport::TransportCatalogue& catalogue
                            , ostream& out
...
//            renderer.SetRenderSettings(GetRenderSettings());
            renderer.SetCanvas(catalogue.GetBusPtrs());
